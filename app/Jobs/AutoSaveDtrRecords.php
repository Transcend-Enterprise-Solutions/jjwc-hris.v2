...
class AutoSaveDtrRecords implements ShouldQueue
{
    ...
    public function __construct()
    {
        ...
        // currentDate used by various methods; initialize to today's date (YYYY-MM-DD)
    ...
    protected function processAllEmployees(): void
    {
        User::where('user_role', 'emp')->chunk(100, function ($users) {
            foreach ($users as $user) {
                $this->processUserDtr($user, $this->currentDate);
            }
        });
    }
    ...
    protected function processUserDtr(User $user, string $currentDate): void
    {
        ...
        $isWFH = $this->isWorkFromHomeDay($schedule, Carbon::parse($currentDate));
    ...
    protected function logJobStart(): void
    {
        ...
        echo "Processing date: {$this->currentDate}\n";
        Log::info("Processing date: {$this->currentDate}");
    }
